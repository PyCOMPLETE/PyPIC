...
\begin{equation}
\rho_{2LR}(x)= 0 \quad \text{for } L<x<2L 
\end{equation}
...
\begin{equation}
\hat{G}_k^\text{int} = \text{DFT}\left\{ G_{2LR}^\text{int}(x_n)\right\}
\end{equation}
...
\item We build its replicated integrated version:

[SORT THIS OUT!!!!!]
\begin{align}
&G_{2LR}^\text{int}(x_n) = h_x G(x_n) \quad \text{for } 0<x_n<L\\
&G_{2LR}^\text{int}(x_n) = G^\text{int}_{2LR}(2L-x) = G(-x) = G(x) 
\end{align}
\item We transform it:
\begin{equation}
{G}_k = \text{DFT}\left\{ G_{2LR}(x_n)\right\}
\end{equation}
\item To obtain $\rho_{2LR}(x_n)$ over the interval $[0, 2L]$ it is enough to extend with zeros the sequence defined in the interval $[0, L]$.
\item We transform:
\begin{equation}
{\rho}_k = \text{DFT}\left\{ \rho_{2LR}(x_n)\right\}
\end{equation}
\item We compute the potential:
\begin{equation}
\phi_{2LR}(x_n) 
=\text{DFT}^{-1}\left\{
