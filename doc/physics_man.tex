...
\begin{equation}
\phi_{2LR}(x) = \frac{1}{2L}\sum_{k=-\infty}^{+\infty} \hat{\phi}_k\, e^{j2\pi k \frac{x}{2L}}
\label{eq:phifour}
\end{equation}
where the Fourier coefficient are given by:

\begin{equation}
\hat{\phi}_k = \int_0^{2L} \phi_{2LR}(x)\, e^{-j2\pi k \frac{x}{2L}} \, dx
\label{eq:phik}
\end{equation}

We replace Eq.\,\eqref{eq:confin} into Eq.\,\eqref{eq:phik} obtaining:
\begin{equation}
\hat{\phi}_k = \int_0^{2L} \int_{0 }^{2L} 
\rho_{2LR}(x')\,G_{2LR}(x-x') dx'\, e^{-j2\pi k \frac{x}{2L}} \,  dx'\, dx
\end{equation}
...
\begin{align}
\hat{\rho}_k = \int_0^{2L} \rho_{2LR}(x)\, e^{-j2\pi k \frac{x}{2L}} \, dx\\
\hat{G}_k = \int_0^{2L} G_{2LR}(x)\, e^{-j2\pi k \frac{x}{2L}} \, dx\\
\end{align}
obtaining simply:
\begin{equation}
\hat{\phi}_k = \hat{G}_k \, \hat{\rho}_k
\end{equation}
